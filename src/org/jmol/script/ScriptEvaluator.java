/* $RCSfile$
 * $Author: hansonr $
 * $Date: 2012-05-20 09:27:14 -0300 (Sun, 20 May 2012) $
 * $Revision: 17177 $
 *
...
public class ScriptEvaluator {
  ...
  public void popContext(boolean isFlowCommand, boolean statementOnly) {
    if (thisContext == null)
      return;
   ...
   * @throws ScriptException
   */
  public void instructionDispatchLoop(boolean doList) throws ScriptException {
    long timeBegin = 0;
    vProcess = null;
    ...
    for (; pc < aatoken.length && pc < pcEnd; pc++) {
      ...
      if (Token.tokAttr(theToken.tok, Token.shapeCommand))
        ...
        case Token.write:
          write(null);
          break;
        case Token.radicalanalyzer:
          System.out.println("case Token.radicalanalyzer ");
          radicalanalyzer();
          break;
        case Token.loadradicalanalyzer:
          System.out.println("case loadradicalanalyzer");
          loadradicalanalyzer();
          break;
        case Token.generategaussianinputfile:
          System.out.println("case Token.generategaussianinputfile");
          System.out.println(RadicalAnalyzer.generateGaussianInputFile(viewer.getModelSet()));
          break;
        case Token.generatesurfaceverticesfile:
          System.out.println("case Token.generategaussianinputfile");
          System.out.println(RadicalAnalyzer.generateIsosurfaceVerticesFile(viewer));
          break;
        case Token.zap:
          zap(true);
          break;
...
//    }
  ...
  */
  ...
  private void radicalanalyzer(){
    System.out.println("RadicalAnalyzer");
    
    //generate mesh tobe used during the computational stage
    if (this.compileScriptString("isosurface resolution 2.5 molecular dots mesh nofill", false))
      try{ 
        this.instructionDispatchLoop(false);
      }catch(ScriptException e){
        System.out.println("error");//totally not needed
      }

    popContext(false, false);//what does this do?
    
    System.out.println("radical analyzer return value:  "+RadicalAnalyzer.generateIsosurfaceGaussianFiles(viewer));
  ...
  public void run(String command){
    ...
    //generate mesh tobe used during the computational stage
    if (this.compileScriptString(command, false))
      try{ 
        this.instructionDispatchLoop(false);
      }catch(ScriptException e){
        System.out.println("error");//totally not needed
  ...
  private void loadradicalanalyzer(){
    System.out.println("LoadRadicalAnalyzer");
    
    //generate mesh tobe used during the computational stage
    if (this.compileScriptString("isosurface resolution 5 molecular dots mesh", false))
      try{ 
        this.instructionDispatchLoop(false);
      }catch(ScriptException e){
        System.out.println("error");//totally not needed
    ...
    RadicalAnalyzer.loadRadicalAnalyzerGaussian(viewer);
